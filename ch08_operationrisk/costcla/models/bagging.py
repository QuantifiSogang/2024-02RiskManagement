...
def _parallel_build_estimators(n_estimators, ensemble, X, y, cost_mat,
    ...
    if (not isinstance(max_samples, (numbers.Integral, int)) and
            (0.0 < max_samples <= 1.0)):
        max_samples = int(max_samples * n_samples)

    if (not isinstance(max_features, (numbers.Integral, int)) and
...
class BaseBagging(with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def fit(self, X, y, cost_mat, sample_weight=None):
        ...
        if isinstance(self.max_samples, (numbers.Integral, int)):
        ...
        if isinstance(self.max_features, (numbers.Integral, int)):
